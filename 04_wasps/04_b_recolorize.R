...
imageList_aligned <- readRDS("04_wasps/rds_files/imageList_aligned.rds")
...
saveRDS(wasp_palette, "04_wasps/rds_files/palette.rds")
...
impose_list <- lapply(imgs, function(i) imposeColors(i, wasp_palette, 
                                                     ...
                                                     plotting = FALSE))
# check visually:
layout(matrix(1:20, nrow = 4)); par(mar = rep(0, 4))
lapply(impose_list, \(x) plotImageArray(recoloredImage(x)))

# save:
saveRDS(impose_list, "04_wasps/rds_files/recolorize_fits.rds")
...
# and save
saveRDS(patternize_list, "04_wasps/rds_files/patternize_list.rds")
